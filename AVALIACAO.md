...
- **Métricas + dashboard da banca (11/07, ADR-008)** — segunda camada do opcional de observabilidade: `/q/metrics` (Micrometer/Prometheus) nos 3 serviços com contadores de negócio de baixa cardinalidade (tags só enumeradas — LGPD), built-ins reusados (cache hit/miss do Caffeine, `ft_*` do SmallRye FT — a consulta não ganhou 1 linha de Java) e `docker compose --profile observabilidade up` subindo Prometheus + Grafana com dashboard provisionado por arquivo (`infra/observabilidade/`). Testes de presença no plano B (`Metricas*Test`); nomes `ft_*` conferidos contra o `/q/metrics` real do plano A.
