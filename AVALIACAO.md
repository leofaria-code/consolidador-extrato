...
> **Como estas notas nasceram:** propostas em sessão de IA (10/07), reavaliadas após o hardening e **ratificadas pelo grupo** na mesma noite. Escala 0–100 por critério (independente do peso) — **não há escala oficial da rubrica documentada no material do curso**, então não é possível confirmar se é a mesma que a banca usa; os pesos por critério (que somam 100) vêm do enunciado do projeto (`CLAUDE.md`). Trajetória: os critérios 4 e 5 começaram em 85 **com gaps nomeados** — e em vez de argumentar por nota, o grupo **fechou os gaps com código e prova ao vivo** (disjuntor+última-boa+DLQ scriptada; broadcast de invalidação com 2 instâncias). O 100,0 é ousado de propósito: cada critério tem evidência executável e os trade-offs remanescentes estão documentados como decisão, não como falta. A banca calibra; a defesa é a evidência.
...
3. **Idempotência e consistência** — peso 12 · nota proposta **100/100**
   ...
   - **Idempotência MEDIDA (11/07, ADR-008)**: `extrato_consolidacao_lancamentos_total{resultado="repetido"}` — a taxa de duplicados absorvidos é uma série no Prometheus, visível no dashboard da banca. `MetricasConsolidacaoTest`: o mesmo lançamento 2× produz as séries `incorporado` E `repetido`.
...
5. **Resiliência** — peso 12 · nota proposta **100/100** *(85 → 95 → 100; os dois gaps nomeados foram fechados em 11/07)*
   ...
   - **Resiliência MEDIDA (11/07, ADR-008)**: DLQ por motivo (`extrato_consolidacao_dlq_enviados_total{motivo=processamento|deserializacao}`), retentativas (`ft_retry_*`) e disjuntor/fallback (`ft_circuitbreaker_state_current`, `ft_invocations_total{fallback="applied"}`) são séries no Prometheus — o painel de DLQ fica **vermelho ao vivo** quando o veneno entra, na frente da banca.
...
6. **Testabilidade** — peso 13 · nota proposta **100/100**
   Evidência:
   - `mvn verify -Pplano-b-jvm` verde sem Docker: 5 módulos, **41 testes, 0 falhas** (reverificado 11/07, pós-métricas) — e **rodando em CI a cada PR** (`.github/workflows/verify.yml`, selo verde no repo): a mesma condição da correção, provada continuamente.
...
7. **Decisões arquiteturais** — peso 13 · nota proposta **100/100**
   Evidência: `docs/adr/` — 9 ADRs, todas com alternativas rejeitadas e o porquê, **com índice navegável** (`docs/adr/README.md`: decisão/US/rejeitadas em 1 linha). **Os 5 candidatos da Sessão 6 estão fechados** (o #1 como ADR-002 no bootstrap; os 4 da issue #7 como ADRs 004–007). As ADRs são vivas: notas de evolução datadas registram o que a prática ensinou (004: expurgo×dedup — US-11 exige US-04; 005: retenção da outbox; 007: a 3ª camada de veneno; 003: pact-em-disco × Broker). Rastreabilidade decisão↔fala de stakeholder via `docs/requisitos/`.
...
- **Métricas + dashboard da banca (11/07, ADR-008)** — segunda camada do opcional de observabilidade: `/q/metrics` (Micrometer/Prometheus) nos 3 serviços com contadores de negócio de baixa cardinalidade (tags só enumeradas — LGPD), built-ins reusados (cache hit/miss do Caffeine, `ft_*` do SmallRye FT — a consulta não ganhou 1 linha de Java) e o `docker compose up` padrão subindo Prometheus + Grafana com dashboard provisionado por arquivo (opt-in por profile até a revisão da ADR-008 em 11/07) (`infra/observabilidade/`). Testes de presença no plano B (`Metricas*Test`); nomes `ft_*` conferidos contra o `/q/metrics` real do plano A.
