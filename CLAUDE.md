...
- [x] 11/07 — Incremento 7: métricas + dashboard (branch `feat/inc-7-metricas`, sessão da Sandy): Micrometer/Prometheus nos 3 serviços (`/q/metrics`; 6 counters de negócio, tags só enumeradas — LGPD; consulta 100% built-in: Caffeine + `ft_*`), ADR-008, Prometheus+Grafana no compose sob `--profile observabilidade` (dashboard "visão da banca" provisionado; portas parametrizáveis `PROMETHEUS_PORT`/`GRAFANA_PORT`); **41 testes verdes no plano B**; validado no plano A com tráfego real (DLQ vermelho ao vivo; achado: fallback do FT é `ft_invocations_total{fallback="applied"}`); Newman 27/27; ato bônus no roteiro-banca
