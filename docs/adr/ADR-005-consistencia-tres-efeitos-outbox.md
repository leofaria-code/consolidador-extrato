...
- (−) **Retenção**: eventos publicados são marcados (`publicado_em`), não apagados — a outbox cresce indefinidamente. *Decisão consciente para o MVP:* manter os publicados é trilha de auditoria da publicação (de graça) e a linha é minúscula (referência + timestamps). *Evolução documentada:* um passo de limpeza no próprio `@Scheduled` (apagar publicados com mais de N dias, configurável) — uma linha de JPQL quando a operação pedir.
