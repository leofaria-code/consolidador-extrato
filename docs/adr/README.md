...
| [008 — Métricas](ADR-008-metricas-micrometer-prometheus.md) | Micrometer + /q/metrics (pull); built-in antes de contador custom; tags só enumeradas (LGPD); Prometheus+Grafana no compose (padrão desde a revisão 11/07) | US-12 (opcional); critérios 2/3/5 medidos | OTel agora; MP Metrics; logs-como-métrica |
| [009 — Distribuição por imagem](ADR-009-distribuicao-por-imagem-docker-hub.md) | Segunda via `docker-compose.hub.yml` + `publicar-hub`: roda do Docker Hub com só Docker (sem clone/JDK/Maven); config de observabilidade assada na imagem | execução por terceiros | monólito empacotado; volume de config (obrigaria clonar) |
