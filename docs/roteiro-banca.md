...
- [ ] **`docs/resumo-visual.md` aberto no GitHub** — a cola: 5 diagramas para apontar durante a arguição (mapa de mecanismos Kafka/RabbitMQ/HTTP, transação dos 3 efeitos, ER da dedup/outbox, régua de veneno, cache/disjuntor). O **mapa de mecanismos** é a resposta visual pronta para "por que tópico aqui e fila ali?".
